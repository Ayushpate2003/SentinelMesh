...
SYSCALLS = ["unshare(CLONE_NEWNS)", "setns(fd, CLONE_NEWUSER)", "mount('/proc/1/root', '/mnt/host')"]
...
def main():
    ...
    print(f"Starting container escape simulation. Max events: {MAX_EVENTS}")
    for i in range(MAX_EVENTS):
        event = {
            "event_id": f"evt_{uuid.uuid4().hex[:8]}",
            "timestamp": time.time(),
            "source": f"container-runtime-{i:02d}",
            "event_type": "container_escape",
            "metadata": {
                "syscall": SYSCALLS[i % len(SYSCALLS)],
                "privileged": True,
                "host_mount_access": True,
                "project_id": "sentinel-runtime",
            },
        }
        try:
