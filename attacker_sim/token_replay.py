...
def main():
    ...
    print(f"Starting token replay simulation. Max events: {MAX_EVENTS}")
    for i in range(MAX_EVENTS):
        event = {
            "event_id": f"evt_{uuid.uuid4().hex[:8]}",
            "timestamp": time.time(),
            "source": f"api-gateway-{i:02d}",
            "event_type": "token_replay",
            "metadata": {
                "token_age_hours": 24 + i * 6,
                "reuse_count": 2 + (i % 5),
                "token_id": "tok_stolen_session",
                "project_id": f"proj-auth-{i % 3}",
            },
        }
        try:
